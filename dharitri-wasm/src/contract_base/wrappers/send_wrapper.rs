    ...
    A: SendApi + ManagedTypeApi + StorageReadApi + BlockchainApi,
{
        ...
        D: ManagedInto<A, ManagedBuffer<A>>,
    {
        ...
        if nonce == 0 {
            ...
            self.api.async_call_raw(
                ...
                &ManagedBuffer::new_from_bytes(self.type_manager(), DCT_TRANSFER_STRING),
                &arg_buffer,
            )
        } else {
            ...
            self.api.async_call_raw(
                ...
                &BigUint::zero(self.type_manager()),
                &ManagedBuffer::new_from_bytes(self.type_manager(), DCT_NFT_TRANSFER_STRING),
                &arg_buffer,
            )
        }
    }
        ...
        D: ManagedInto<A, ManagedBuffer<A>>,
    {
        ...
        self.api.async_call_raw(
            &self.api.get_sc_address(),
            &BigUint::zero(self.type_manager()),
            &ManagedBuffer::new_from_bytes(self.type_manager(), DCT_MULTI_TRANSFER_STRING),
            &arg_buffer,
        );
    }
    ...
    pub fn dct_nft_create<T: dharitri_codec::TopEncode>(
        ...
        let output = self.call_local_dct_built_in_function(
            self.api.get_gas_left(),
            &ManagedBuffer::new_from_bytes(self.type_manager(), b"DCTNFTCreate"),
            &arg_buffer,
        );
